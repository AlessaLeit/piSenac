...
from entidades import funcionario as mod_funcionario
from entidades import cliente as mod_cliente
from entidades import produto
from entidades import servico
from entidades import agenda as mod_agenda
from entidades import venda as mod_venda
from entidades import fornecedor as mod_fornecedor
from entidades import despesa as mod_despesa
from entidades import suprimento as mod_suprimento
from entidades import maquina as mod_maquina
...
def _cadastrar_agendamento_ui():
   ...
   try:
       mod_agenda.Agenda.criar_agenda(
           ...
           mod_agenda.StatusAgenda.AGENDADO
       )
...
def _filtrar_agendamentos_ui():
   ...
   if solicitar_sim_nao("Deseja filtrar por Status da Agenda?") == True:
       ...
       if status_str is None:
           ...
           try:
               status_filtro = mod_agenda.StatusAgenda[status_str.upper().replace(" ", "_")]
           except ValueError as e:
               print(f"Erro: {e}")
               return
...
def _marcar_agendamento_realizado_ui():
   ...
   if agenda.status == mod_agenda.StatusAgenda.REALIZADO:
...
def _marcar_agendamento_nao_realizado_ui():
   ...
   if agenda.status == mod_agenda.StatusAgenda.NAO_REALIZADO:
...
def _cadastrar_venda_ui():
   ...
   if agenda_resp:
       while True:
           ...
           if temp_agenda_obj:
               if temp_agenda_obj.status == mod_agenda.StatusAgenda.REALIZADO:
   ...
   try:
       ...
       if agenda_linked_during_process and agenda_obj:
           ...
           try:
               ...
               agenda_obj.status = mod_agenda.StatusAgenda.AGENDADO # Reverte para AGENDADO para que possa ser reutilizada
       ...
       if agenda_linked_during_process and agenda_obj:
           ...
           try:
               agenda_obj.marcar_como_nao_realizado()
               agenda_obj.status = mod_agenda.StatusAgenda.AGENDADO # Reverte para AGENDADO
               print(f"Agendamento ID {agenda_obj.id} revertido para AGENDADO e estoque restaurado.")
           except Exception as revert_e:
                print(f"Erro adicional ao tentar reverter agendamento ID {agenda_obj.id}: {revert_e}")
...
def _atualizar_venda_ui():
   ...
   if venda_existente.agenda and venda_existente.agenda.status == mod_agenda.StatusAgenda.REALIZADO:
           ...
           if venda_existente.agenda and venda_existente.agenda.status == mod_agenda.StatusAgenda.REALIZADO:
               if solicitar_sim_nao("Ao remover a agenda associada, deseja reverter o status da agenda para AGENDADO e reverter os estoques?") == True:
                   try:
                       ...
                       venda_existente.agenda.status = mod_agenda.StatusAgenda.AGENDADO # Garante que o status é AGENDADO após reversão
